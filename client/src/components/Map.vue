    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        ...
        <v-card-text>
          ...
          <div v-if="allowedDates !== undefined">
            <v-date-picker
              ...
              class="mt-3"
              v-validate="'required'" :error-messages="errors.collect('dateSelection')" data-vv-name="dateSelection" required></v-date-picker>
          </div>
          <div v-else>
            <v-date-picker
              v-model="date"
              ...
              landscape
              class="mt-3"
              v-validate="'required'" :error-messages="errors.collect('dateSelection')" data-vv-name="dateSelection" required
            ></v-date-picker>
          </div>
          <br/>
          <v-select v-model="moment" :items="items" v-validate="'required'" :error-messages="errors.collect('select')" label="Outline style"
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" flat @click="submit()">Add to my planning</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    submit () {
      ...
      if (!valid || this.allowedDates(this.date) == false) {
        alert("Unable to add to planning, there is an error somewhere");
      } else {
        thisRef.addToPlanning();
      }
    });
